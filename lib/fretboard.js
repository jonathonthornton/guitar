// Shared fretboard primitives used by modes.html, scales.html, and
// arpeggio-position.html: standard tuning/geometry constants, the CAGED
// pentatonic box shapes (and the triad-filtered shapes built on top of
// them), the low-level SVG string builder, the "finalize a shape" /
// "render a shape" helpers, the full-fretboard renderer, and the shared
// "render a scale/mode's whole output panel" orchestrator (used by
// modes.html/scales.html only — arpeggio-position.html's output is grouped
// per scale-degree, so it builds its own). Depends on noteIndex/NOTES from
// theory.js. Each page keeps its own higher-level shape computation
// (computeModalBoxes vs. computeShapes vs. computeArpeggioShapes),
// buildNoteInfo, and degreeColor (different palettes) local, since those
// differ — this file only holds what pages do identically.
...
// The minor pentatonic of a major chord/key's root shares the same five
// boxes as the relative minor (root minus 3 semitones) — this keeps major
// and minor pentatonic (and anything derived from them: triads, arpeggios,
// blues, diatonic fills) on the identical CAGED shapes.
function pentatonicBoxes(rootPc, isMajor) {
  const minorRootPc = isMajor ? ((rootPc - 3) % 12 + 12) % 12 : rootPc;
  return pentatonicBoxesForRoot(minorRootPc);
}

// The same five pentatonic boxes, filtered to only the given chord tones —
// used for both a scale's "Triad" positional shapes (scales.html) and a
// single chord's 1-3-5 arpeggio shapes (arpeggio-position.html, one call
// per scale degree). triadNotes: chromatic pitch-class names to keep.
function computeTriadShapes(rootPc, isMajor, triadNotes, noteInfo) {
  return pentatonicBoxes(rootPc, isMajor).map((box, idx) => {
    const frets = box.frets.map((stringFrets, s) =>
      stringFrets.filter(f => triadNotes.includes(getFretNote(s, f))));
    return finalizeShape(frets, noteInfo, idx);
  });
}

// Adds one out-of-box chromatic note into a pentatonic box's frets — a note
// that isn't one of the box's own 2-notes-per-string pattern, sitting a
// semitone off from one of its regular degrees (scales.html's blues b5, or
// a diminished triad's b5, which is a tritone from the root and so never
// falls on the minor-pentatonic box's own notes either). Scans one fret
// beyond the box's own edges on each string, keeps at most one occurrence
// per string (preferring whichever is adjacent to a note already in the
// box, so it reads as a natural extension of the shape), and never lets it
// become the position's own lowest fret — that would shift the shape's
// apparent anchor a fret to the left of where the box itself starts.
function addOutOfBoxNote(boxFrets, targetNote) {
  const allFrets = boxFrets.flat();
  const lo = Math.min(...allFrets), hi = Math.max(...allFrets);

  const candidates = [];
  for (let s = 0; s < NUM_STRINGS; s++) {
    for (let f = Math.max(0, lo - 1); f <= hi + 1; f++) {
      if (getFretNote(s, f) === targetNote && !boxFrets[s].includes(f)) {
        const adjacent = boxFrets[s].some(bf => Math.abs(bf - f) === 1);
        candidates.push({ s, f, pitch: OPEN_ABS[s] + f, adjacent });
      }
    }
  }
  const chosenByPitch = new Map();
  for (const c of candidates) {
    const existing = chosenByPitch.get(c.pitch);
    if (!existing || (c.adjacent && !existing.adjacent)) chosenByPitch.set(c.pitch, c);
  }
  const byString = {};
  for (const c of chosenByPitch.values()) {
    (byString[c.s] = byString[c.s] || []).push(c.f);
  }

  let overallMin = lo, overallMinString = null;
  for (const [sKey, fretsArr] of Object.entries(byString)) {
    for (const f of fretsArr) {
      if (f < overallMin) { overallMin = f; overallMinString = sKey; }
    }
  }
  if (overallMinString !== null) {
    const arr = byString[overallMinString];
    arr.splice(arr.indexOf(overallMin), 1);
  }

  return boxFrets.map((stringFrets, s) => {
    const combined = [...stringFrets, ...(byString[s] || [])];
