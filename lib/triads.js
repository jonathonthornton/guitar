...
function drawTriadDiagram(voicing) {
  ...
  // Nut or fret number
  if (hasNut) {
    svg.push(`<rect x="${mL}" y="${mT}" width="${gridW}" height="4" fill="#e0a940"/>`);
  } else {
    svg.push(`<text x="${mL - 12}" y="${mT + fsp * 0.7}" text-anchor="end" font-family="DM Mono,monospace" font-size="10" fill="#7488ac">${startFret}fr</text>`);
  }
  ...
  for (let f = 0; f <= nFrets; f++) {
    ...
    const y = topY + f * fsp;
    svg.push(`<line x1="${mL}" y1="${y}" x2="${mL + gridW}" y2="${y}" stroke="#2c3a5e" stroke-width="1"/>`);
  }
  ...
  for (let s = 0; s < nStr; s++) {
    const x = mL + s * ssp;
    svg.push(`<line x1="${x}" y1="${topY}" x2="${x}" y2="${bottomY}" stroke="#4a5d85" stroke-width="1.2"/>`);
  }
  ...
  stringLabels.forEach((n, i) => {
    svg.push(`<text x="${mL + i * ssp}" y="${H - 4}" text-anchor="middle" font-family="DM Mono,monospace" font-size="9" fill="#4a5d85">${n}</text>`);
  });
