// String-set picker: a 6-string headstock/fretboard diagram, used by
// chords.html. Draws the tuners, fanned strings, and fretboard, then lets
// the user hover/click one of four overlapping 3-string groups (EAD, ADG,
// DGB, GBe).
//
...
// mount element carrying class="string-picker" (default id
// "stringset-control" — chords.html already uses that id to show/hide this
// control when switching restrict-by mode), and buildStringSetPicker()
// builds the whole board-wrap/svg/readout skeleton into it — headstock
// shape, nut, fretboard base rect, catcher rect and all — injecting this
// widget's stylesheet at the same time.

const STRING_PICKER_CSS = `
.string-picker {
  --fb-bg: #17110d;
...
// The static board skeleton buildStringSetPicker() builds once into the
// mount element — headstock shape, nut, fretboard base rect, catcher rect —
// everything the rest of this file's JS then fills in (grain, tuners,
// fanned strings, frets, strings, labels) by id.
const STRING_PICKER_SKELETON = `
  <div class="board-wrap">
    <svg id="board" viewBox="0 0 468 560" xmlns="http://www.w3.org/2000/svg">

      <!-- ===== headstock ===== -->
      <g id="headstock">
        <path id="headstockShape" d="M 151.2 46
                 Q 151.2 30 166.5 30
                 L 301.5 30
                 Q 316.8 30 316.8 46
                 L 309.6 280
                 ...
                 Z" fill="var(--fb-rosewood-mid)" stroke="var(--fb-rosewood-dark)" stroke-width="2" />
        <!-- subtle grain -->
        ...
        <g id="logoText" font-family="Snell Roundhand, 'Brush Script MT', cursive, Georgia" fill="var(--fb-gold)">
        </g>
  ...
  <div class="readout">
    <span class="value" id="readoutValue"></span>
  </div>
`;
...
// Builds the board skeleton into the caller's (empty) `containerId` element,
// then the headstock logo, tuners, fretboard, and interactive string groups
// inside that. onSelect(stringSetKey) fires whenever the user clicks a
// string group; defaultGroup (an index into ['EAD','ADG','DGB','GBe']) is
// locked in immediately.
function buildStringSetPicker({ containerId = 'stringset-control', onSelect, defaultGroup = 3 } = {}) {
  injectStyleOnce('string-picker-styles', STRING_PICKER_CSS);
  document.getElementById(containerId).innerHTML = STRING_PICKER_SKELETON;
  ...
  readout.textContent = groupLabel(defaultGroup);
}
