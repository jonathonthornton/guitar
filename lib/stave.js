// Shared treble-clef stave renderer used by modes.html, scales.html, and
// triads.html. Depends on createSVGElement/injectStyleOnce from
// svg-utils.js, and on each page's own degreeColor(index, degreeNames) — the
// note-color palette differs per page (modes.html's modal degree names vs.
// scales.html/triads.html's scale/chord degree names), so that lookup stays
// page-local rather than living here.
//
// This module owns its own markup and CSS: the caller just provides an empty
// mount element (default id "stave-mount"), and renderMusicStave() creates
// the <svg id="key-stave"> and <div class="stave-label"> inside it the first
// time it's called, injecting this widget's stylesheet at the same time.
...
function renderMusicStave({
  ...
  injectStyleOnce('stave-styles', STAVE_CSS);

  let svg = document.getElementById(svgId);
  let label = document.getElementById(labelId);
  if (!svg) {
    const container = document.getElementById(containerId);
    svg = createSVGElement('svg', { id: svgId, viewBox: '0 0 560 173.33' });
    container.appendChild(svg);
    label = document.createElement('div');
    label.className = 'stave-label';
    label.id = labelId;
